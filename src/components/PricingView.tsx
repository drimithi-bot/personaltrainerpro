    ...
    <div className="flex-1 bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-blue-500/50 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 p-6 md:p-8 overflow-y-auto flex items-center justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 w-full max-w-7xl mx-auto">
          {plans.map((plan) => (
            <div key={plan.id} className={`relative bg-white dark:bg-slate-900 rounded-3xl p-5 border-2 transition-all hover:-translate-y-1 flex flex-col ${plan.popular ? 'border-indigo-500 shadow-md' : 'border-slate-100 hover:border-indigo-200'}`}>
              {plan.popular && (
              ...
              <ul className="flex flex-col gap-2 mb-4 text-xs text-slate-600 flex-1">
              ...
              <button 
                ...
                className={`w-full py-2 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-colors mt-auto ${plan.popular ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 text-slate-700 border border-slate-200'}`}
              >
